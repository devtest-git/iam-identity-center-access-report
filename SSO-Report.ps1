...
$SSOParams   = $OrgParams.Clone()
$IdsParams = $OrgParams.Clone()
...
foreach ($Account in $AccountList) {
    ...
    foreach ($PS in Get-SSOADMNPermissionSetsProvisionedToAccountList -AccountId $Account.Id @SSOParams) {
        if (-not $PSsets[$PS]) {
            $PSsets[$PS] = (Get-SSOADMNPermissionSet @SSOParams -PermissionSetArn $PS).Name
            $APICalls++
        }
        ...
        if ($AssignmentsResponse.NextToken) {
            ...
            $AccountAssignments = $AssignmentsResponse
        }
        ...
        foreach ($Assignment in $AccountAssignments) {
            if (-not $Principals[$Assignment.PrincipalId]) {
                ...
                # Add error handling for deleted users and groups
                try {
                    ...
                    if ($Assignment.PrincipalType.Value -eq "GROUP") {
                        ...
                        $Principals[$Assignment.PrincipalId] = $Principal.UserName
                    }
                }
                catch [Amazon.IdentityStore.Model.ResourceNotFoundException] {
                    # Handle deleted users or groups
                    Write-Warning "$AssignmentType not found: $($Assignment.PrincipalId) - $AssignmentType has been deleted from identity source"
                    
                    if ($Assignment.PrincipalType.Value -eq "GROUP") {
                        $Principals[$Assignment.PrincipalId] = "DELETED_GROUP_$($Assignment.PrincipalId)"
                    }
                    else {
                        $Principals[$Assignment.PrincipalId] = "DELETED_USER_$($Assignment.PrincipalId)"
                    }
                }
                catch {
                    # Handle any other unexpected errors
                    Write-Warning "Error retrieving $AssignmentType $($Assignment.PrincipalId): $($_.Exception.Message)"
                    $Principals[$Assignment.PrincipalId] = "ERROR_$($AssignmentType)_$($Assignment.PrincipalId)"
                }
            }
